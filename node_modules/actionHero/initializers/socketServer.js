...
var socketServer = function(api, next){
  ...
  if(api.configData.tcpServer.enable != true){
    ...
    api.socketServer.parseRequest = function(connection, line){
      ...
      if(words[0] == "leaveRoom"){
		//to leave a room first remove self from room hashes
		// remove room from users connected rooms
		api.chatRoom.roomRemoveMember(connection, function(err, wasRemoved){
			var roomToLeave = words[1];
			api.chatRoom.leaveRoom(roomToLeave,connection,function(err,removed){
				api.socketServer.prepareSocketMessage(connection, {context: "response", status: "OK", current_room: connection.room});
		        api.log("[ leaveRoom @ socket ]", 'debug', {to: connection.remoteIP, params: JSON.stringify(words)});	
			});	
			connection.room = "defaultRoom";
...
//          api.socketServer.prepareSocketMessage(connection, {context: "response", status: "OK", room: connection.room});
	...
	console.log("calling goodbye, ie. leave chat");
      try{ 
